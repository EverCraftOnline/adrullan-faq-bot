...
module.exports = {
  ...
  async listWorkspaceFiles() {
    try {
      ...
      console.log(`📁 Found ${response.data?.length || 0} files in workspace`);
...
Respond using only the provided context. If the answer isn't in the context, say "I don't have official information about that in our current FAQ." Be helpful, accurate, and cite your sources.`;
  ...
  async askWithFiles(systemPrompt, question, fileIds = []) {
    try {
      // If specific files provided, use those; otherwise let Anthropic choose
      let filesToUse = fileIds.length > 0 ? fileIds : [];
      
      if (filesToUse.length === 0) {
        // Check if we have any uploaded files, but don't force them all
        const availableFiles = Array.from(uploadedFiles.values()).flat();
        if (availableFiles.length === 0) {
          throw new Error('No files uploaded to Anthropic workspace');
        }
        console.log(`🤖 Using Files API - letting Anthropic choose from ${availableFiles.length} available files`);
      } else {
        console.log(`🤖 Using Files API with ${filesToUse.length} specified files`);
      }
      ...
      // Only add file references if we specifically want to use certain files
      if (filesToUse.length > 0) {
        messageContent.push(...filesToUse.map(fileId => ({
          type: 'document',
          source: {
            type: 'file',
            file_id: fileId
      ...
      // Load knowledge base and get relevant context (from data/ directory only)
      const knowledgeLoader = require('./knowledgeLoader');
      const knowledgeBase = await knowledgeLoader.loadAll();
      
      if (knowledgeBase.length === 0) {
        throw new Error('No knowledge base found for fallback');
      }
      
      // Import the context building function from askCommand
      const { buildComprehensiveContext } = require('../commands/askCommand');
      
      // Get relevant context using the same logic as askWithContext
      const relevantContext = buildComprehensiveContext(question, knowledgeBase, 12000);
