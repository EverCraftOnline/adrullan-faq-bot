...
class BotMonitor {
  ...
  getCostMetrics() {
    ...
    const todayCost = safeNumber(this.dailyCost.get(today) || 0);
    
    return {
      total: {
        tokens: safeNumber(this.totalTokensUsed),
        cost: safeNumber(this.totalCost),
        requests: safeNumber(this.requestCount)
      },
      daily: {
        today: {
          tokens: safeNumber(this.dailyTokens.get(today) || 0),
          cost: todayCost
        },
        yesterday: {
          tokens: safeNumber(this.dailyTokens.get(yesterday) || 0),
          cost: safeNumber(this.dailyCost.get(yesterday) || 0)
        }
      },
      average: {
        costPerRequest: this.requestCount > 0 ? safeNumber(this.totalCost / this.requestCount) : 0,
        tokensPerRequest: this.requestCount > 0 ? safeNumber(this.totalTokensUsed / this.requestCount) : 0
      },
      estimated: {
        monthlyCost: safeNumber(todayCost * 30), // Rough estimate - now safe from NaN
        tokensPerDay: safeNumber(Object.values(Object.fromEntries(this.dailyTokens)).reduce((a, b) => a + b, 0) / Math.max(1, this.dailyTokens.size))
      }
    };
  }
